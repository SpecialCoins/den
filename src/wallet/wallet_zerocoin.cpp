    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZBCZSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZBCZSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zbczseed to wallet";
    }
    //the use case for this is no password set seed, mint dzBCZ,
...
bool CWallet::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZBCZSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZBCZSeed(hashSeed, vchSeed)) {
...
bool CWallet::CreateZBCZOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZBCZ(denomination, coin, dMint);
                                            ...
                                            const CCoinControl* coinControl)
{
    ...
    while (true) {
        ...
        if (!CreateZBCZOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zbcz output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to Bcz
        ...
        std::list<std::pair<CTxDestination, CAmount>> addressesTo, CBitcoinAddress* changeAddress)
{
    ...
    int nStatus = ZBCZ_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZBCZ_WALLET_LOCKED);
        return false;
    }
    ...
    if (res.status != CWallet::CommitStatus::OK) {
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZBCZ_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZBCZ_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zbczTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zbczTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZBCZ_SPEND_OKAY);  // When we reach this point spending zBCZ was successful
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZBCZ_TXMINT_GENERAL);
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
            receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZBCZ_TXMINT_GENERAL);
            return false;
        } else if (mapBlockIndex.count(hashBlock) < 1) {
            // check that this mint made it into the blockchain
            receipt.SetStatus(_("Mint did not make it into blockchain"), ZBCZ_TXMINT_GENERAL);
            return false;
        }
        ...
        if (outputIndex == -1) {
            receipt.SetStatus(_("Pubcoin not found in mint tx"), ZBCZ_TXMINT_GENERAL);
            return false;
        }

        mint.SetOutputIndex(outputIndex);
        CTxIn in;
        if(!ZBCZModule::createInput(in, mint, hashTxOut, spendVersion)) {
            receipt.SetStatus(_("Cannot create public spend input"), ZBCZ_TXMINT_GENERAL);
            return false;
        }
        vin.emplace_back(in);
    ...
    receipt.SetStatus(_("Spend Valid"), ZBCZ_SPEND_OKAY); // Everything okay
        ...
        CBitcoinAddress* changeAddress)
{
    ...
    int nStatus = ZBCZ_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zBCZ"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZBCZ_TRX_CREATE;
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zBCZ public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zBCZ used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zbczTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zBCZ mints to use in this spend
    ...
    for (CZerocoinMint mint : vSelectedMints) {
        ...
        if (IsSerialInBlockchain(mint.GetSerialNumber(), nHeightSpend)) {
            ...
            if (!zbczTracker->HasSerialHash(hashSerial))
                return error("%s: tracker does not have serialhash %s", __func__, hashSerial.GetHex());

            CMintMeta meta = zbczTracker->Get(hashSerial);
            meta.isUsed = true;
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CTxDestination,CAmount> pair : addressesTo){
                ...
                //add output to bcz address to the transaction (the actual primary spend taking place)
                // TODO: check value?
            ...
            unsigned int nBytes = ::GetSerializeSize(txNew, SER_NETWORK, PROTOCOL_VERSION);
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZBCZ_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZBCZ_SPEND_OKAY); // Everything okay
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zBCZ in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zbczTracker->GetMints(true);
    ...
    return zbczTracker->GetBalance(false, false);
}

CAmount CWallet::GetUnconfirmedZerocoinBalance() const
{
    return zbczTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zbczTracker->ListMints(true, true, true);
...
void CWallet::setZWallet(CzBCZWallet* zwallet)
{
    zwalletMain = zwallet;
    zbczTracker = std::unique_ptr<CzBCZTracker>(new CzBCZTracker(this));
}

CzBCZWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
...
{
    return zbczTracker->HasSerial(bnSerial);
}

bool CWallet::IsMyMint(const CBigNum& bnValue) const
{
    if (zbczTracker->HasPubcoin(bnValue))
        return true;
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zbczTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zbczTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zbczTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
...
{
    CMintMeta meta;
    if (!zbczTracker->GetMetaFromStakeHash(hashStake, meta))
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zbczTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zbczTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zbczTracker->UpdateState(meta);
    } else {
