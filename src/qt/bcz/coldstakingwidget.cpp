...
#include "qt/bcz/coldstakingwidget.h"
#include "qt/bcz/forms/ui_coldstakingwidget.h"
...
ColdStakingWidget::ColdStakingWidget(BCZGUI* parent) :
    PWidget(parent),
    ...
    isLoading(false)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your BCZ letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Accept BCZ delegation / Delegate BCZ"));
    ...
    ui->labelEditTitle->setText(tr("Cold Staking address (from remote staker, starts with letter S)"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select BCZ outputs to delegate.");
...
void ColdStakingWidget::updateStakingTotalLabel()
{
    ...
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 BCZ" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}
