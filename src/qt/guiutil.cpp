...
QString formatBalance(CAmount amount, int nDisplayUnit, bool isZbcz)
{
    return (amount == 0) ? ("0.00 " + BitcoinUnits::name(nDisplayUnit, isZbcz)) : BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, amount, false, BitcoinUnits::separatorAlways, true, isZbcz);
}
...
void restoreWindowGeometry(const QString& strSetting, const QSize& defaultSize, QWidget* parent)
{
    ...
    if (!pos.x() && !pos.y()) {
        Rect screen = QGuiApplication::primaryScreen()->geometry();
