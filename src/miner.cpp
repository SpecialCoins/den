...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet)
{
    ...
    if (!pwallet->CreateCoinStake(pindexPrev, pblock->nBits, txCoinStake, nTxNewTime))
    {
    ...
    CBlockTemplate* CreateNewBlockWithKey(CReserveKey& reservekey, CWallet* pwallet)
    {
        ...
        return CreateNewBlock(scriptPubKey, pwallet);
    }
    ...
    void POSMiner(CWallet* pwallet)
    {
        ...
        while (fStake_BCZ) {
            ...
            while (vNodes.empty() || pwallet->IsLocked() || !fStakeableCoins  || (pwallet->GetBalance() > 0 && nReserveBalance >= pwallet->GetBalance()) || !masternodeSync.IsBlockchainSynced())
            {
                MilliSleep(5000);
            ...
            std::unique_ptr<CBlockTemplate> pblocktemplate((CreateNewBlockWithKey(reservekey, pwallet)));
            ...
            while (true) {
                ...
                // Found a solution
                SetThreadPriority(THREAD_PRIORITY_NORMAL);
                LogPrintf("POSMiner : block found  \n  hash: %s  \ntarget: %s\n", hash.GetHex(), hashTarget.GetHex());
